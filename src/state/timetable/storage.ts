 ...
 *
 * The legacy-read loop is intentionally sequential, not parallel: each
 * `getStoredValue` is a fast local-storage lookup (or an AsyncStorage
 * read on native, still microseconds) and the original monolithic
 * implementation did the same. If profiling later shows this matters,
 * the loop can become `Promise.any` over wrapped rejects.
 */
export async function loadSnapshot(): Promise<TimetableSnapshot> {
  const current = await getStoredValue(STORAGE_KEY);
  if (current) {
  ...
  for (const legacyKey of LEGACY_KEYS) {
    const raw = await getStoredValue(legacyKey);
    if (!raw) continue;
    ...
    await setStoredValue(legacyKey, '');
    return snapshot;
  }
